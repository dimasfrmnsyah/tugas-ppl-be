...
const { _status } = require('../constants')
...
const getFilterLk = async (fd, isAggregate) => {
    ...
    if (fd[i].id === 'clusterTruck') {
      const cluster = await Cluster.findOne({ _id: fd[i].value })
      const {truckOption} = cluster
      // if (truckOption.includes('fromLocation')) condition['statusCode'] = { '$in': [
      //   _status.truck.onDuty.statusCode,
      //   _status.truck.empty.statusCode,
      // ] }
      // if (truckOption.includes('toLocation')) condition['statusCode'] = { '$in': [
      //   _status.truck.onDuty.statusCode,
      //   _status.truck.empty.statusCode,
      // ] }
    }
...
const getQuery = async (query, isAggregate = false) => {
    ...
    // console.log(filteredLk, 'ini filter')
